...
class MauticPatchTester
{
    ...
    public function __construct()
    {
        ...
        if (in_array($task, $allowedTasks)) {
            ...
            foreach ($cmd as $c => $value) {
                $counter++;
                file_put_contents(__DIR__.'/progress.json',
                    json_encode(['progress' => floor((100 / $count) * $counter)]));
    ...
    function apply($patch = null)
    {
        if ($patch) {
            ...
            $result = shell_exec('curl '.$patchUrl.' | git apply   2>&1');
        } else {
            $result = 'Apply with no Patch ID';
        }
        echo nl2br($result);
    }
    ...
    function remove($patch = null)
    {
        if ($patch) {
            $patchUrl = $this->url.$patch.'.patch';
            $result = shell_exec('curl '.$patchUrl.' | git apply -R  2>&1');
        } else {
            $result= 'Could not remove Patch';
        }
    ...
    { ?>
        ...
        <div class="container">
            <nav class="navbar navbar-light bg-light navbar-expand-lg">
                <a class="navbar-brand" href="https://www.mautic.org">
                    <img src="https://www.mautic.org/media/logos/notagline/horizontal/Mautic_Logo_RGB_LB.png"
                         height="30" class="d-inline-block align-top" alt="">
                </a>
                <div class="collapse navbar-collapse" id="navbarNav">
                    <ul class="nav navbar-nav">
                        <li class="nav-item"><a class="nav-link"
                                                href="https://github.com/mautic/mautic/pulls?q=is%3Apr+is%3Aopen+label%3A%22Ready+To+Test%22"
                                                target="_blank">GitHub PR's</a></li>
                    </ul>
                </div>
            </nav>
            ...
            <div class="container mt-4">
                <h1>Start Testing!</h1>
                <p class="lead">This app will allow you to immediately begin testing pull requests against your Mautic
                    installation. You will need to make sure this file is in the root of your Mautic test instance.</p>
                ...
                <div id="resultBar">
                    ...
                    <div class="case alert alert-danger">
                    </div>
                </div>
            </div>
                            ...
                            <div class="card-body">
                                ...
                                <div class="card-text">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="checkbox">
                                            <label>
                                                <input type="checkbox" class="cmd" id="cache-clear"
                                                       name="cmd[cache:clear]" value="1">
                                                <label for="cache-clear">clear cache</label>
                                            </label>
                                        </div>
                                        <div class="checkbox">
                                            <label>
                                                <input class="cmd" type="checkbox" id="dsu"
                                                       name="cmd[doctrine:schema:update --force]" value="1">
                                                <label for="dsu">doctrine:schema:update --force</label>
                                        </div>
                                        <div class="checkbox">
                                            <label>
                                                <input type="checkbox" id="mag" class="cmd"
                                                       name="cmd[mautic:assets:generate]" value="1">
                                                <label for="mag">mautic:assets:generate</label>
                                                <small style="color:red">(it can take a few minutes)</small>
                                            </label>
                                        </div>
                                    </div>
                                    <span class="pt-3 text-small float-right text-success"
                                          id="pr-applied-message"></span>
                                    <input type="submit" id="apply" class="btn btn-success btn-lg"
                                           data-loading-text="Applying..." value="Apply PR"/>
                                </div>
                            </div>
                        </div>
                    </form>
                            ...
                            <div class="card-body">
                                ...
                                <div class="card-text">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="checkbox">
                                            <label>
                                                <input type="checkbox" class="cmd" id="cache-clear-r"
                                                       name="cmd[cache:clear]" value="1">
                                        ...
                                        <div class="checkbox">
                                            <label>
                                                <input class="cmd" type="checkbox" id="dsu-r"
                                                       name="cmd[doctrine:schema:update --force]" value="1">
                                                <label for="dsu-r">doctrine:schema:update --force</label>
                                        </div>
                                        <div class="checkbox">
                                            <label>
                                                <input type="checkbox" id="mag-r" class="cmd"
                                                       name="cmd[mautic:assets:generate]" value="1">
                                                <label for="mag-r">mautic:assets:generate</label>
                                                <small style="color:red">(it can take a few minutes)</small>
                                            </label>
                                        </div>
                                        <span class="pt-3 text-small float-right text-danger"
                                              id="pr-removed-message"></span>
                                        <input type="submit" id="remove" class="btn btn-success btn-lg"
                                               data-loading-text="Removing..." value="Remove PR"/>
                                    </div>
                                </div>
                            </div>
                    </form>
        ...
        <script type="text/javascript">
            ...
            jQuery(document).ready(function () {
                jQuery("#progresBar,#resultBar").hide();
                jQuery('#apply-form').on("submit", function (e) {
                    ...
                    jQuery.ajax({
                        'url': './<?php echo $this->testerfilename ?>',
                        ...
                        .done(function (result) {
                            if(result) {
                                jQuery("#resultBar").show().find('.case').html(result);
                            }
